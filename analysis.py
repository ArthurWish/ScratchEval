...
class HandleExcel():
    ...
    def run_main_save_to_excel_with_openpyxl(self):
        ...
        self.write_to_excel_with_openpyxl(
            dataset_list, head_row_label, save_name)
...
def ctAnalysis(argv):
    ...
    for id in block.keys():
        ...
        if (parent_id == None and id not in overp):
            ...
            if (block[id].name in block_dict.values()):
                point = block_grade[list(block_dict.keys())[list(
                    block_dict.values()).index(block[id].name)]]
    ...
    for id in block.keys():
        ...
        if (block[id].spriteName not in oversprite):
            g.add_weighted_edges_from(
                [(block[id].spriteName, "start_flag", 1)])
            oversprite.add(block[id].spriteName)
            save_graph.append(
                {"source": block[id].spriteName, "target": "start_flag", "weight": 1})
        ...
        if not (name.find("when") == -1 and name.find("start") == -1):
            ...
            save_graph.append(
                {"source": block[id].spriteName, "target": idn, "weight": point})
        if not name.find("definition") == -1:
            # print(idn)
            g.add_weighted_edges_from([("start_flag", idn, point)])
            save_graph.append(
                {"source": "start_flag", "target": idn, "weight": point})
        # 和next和substack建立连接
        if (block[id].getNext() != None):
            g.add_weighted_edges_from(
                [(graphid[block[id].getNext()], idn, point)])
            save_graph.append(
                {"source": graphid[block[id].getNext()], "target": idn, "weight": point})
        if (block[id].getSubstack() != None):
            g.add_weighted_edges_from(
                [(graphid[block[id].getSubstack()], idn, point)])
            save_graph.append(
                {"source": graphid[block[id].getSubstack()], "target": idn, "weight": point})
        if (block[id].getSubstack2() != None):
            g.add_weighted_edges_from(
                [(graphid[block[id].getSubstack2()], idn, point)])
            save_graph.append(
                {"source": graphid[block[id].getSubstack2()], "target": idn, "weight": point})
        # block之间建立连接
            ...
            if (name.find("clone") != -1):
                for id2 in start_clone_set:
                    if (block[id2].getSpriteName() == block[id].getSpriteName()):
                        g.add_weighted_edges_from(
                            [(idn, graphid[id2], block_grade[name])])
                        save_graph.append(
                            {"source": graphid[id2], "target": idn, "weight": block_grade[name]})
...
if __name__ == '__main__':
    ...
    json_string = json.dumps(all_results, ensure_ascii=False, indent=4)
    with open("./graph_results.json", "w", encoding='utf-8') as f:
        f.write(json_string)
